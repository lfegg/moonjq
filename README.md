...
MoonJQ 是一个使用 [MoonBit](https://github.com/moonbitlang/moonbit) 编写的轻量级、交互式 JSON 处理器。提供友好的 REPL 交互环境。
...
- **核心 `jq` 功能**:
  ...
  - **空值合并**: `//` (左侧为 null 或 false 时返回右侧值)
  - **括号**: `(...)` (改变运算优先级)
  - **负数字面量**: `-5`, `-3.14` (支持负数直接使用)
  - **类型函数**: `type` (返回值类型)
  - **类型转换**: `tostring` (转换为字符串), `tonumber` (转换为数字)
  - **工具函数**: `length` (获取长度), `keys` (获取键名或索引)
...
```bash
# // - 空值合并运算符
# 当左侧为 null 或 false 时返回右侧值，否则返回左侧值

jq> echo 'null' | jq '. // "default"'
"default"

jq> echo 'false' | jq '. // "default"'
"default"

jq> echo '0' | jq '. // "default"'
0

jq> echo '""' | jq '. // "default"'
""

jq> echo '5' | jq '. // "default"'
5

# 字段访问中的空值合并
jq> echo '{"a":null,"b":5}' | jq '.a // .b'
5

jq> echo '{"a":false,"b":"hello"}' | jq '.a // .b'
"hello"

jq> echo '{"a":0,"b":10}' | jq '.a // .b'
0

# 链式空值合并
jq> echo '{"a":null,"b":null,"c":"value"}' | jq '.a // .b // .c'
"value"

# 与可选操作符结合
jq> echo '{}' | jq '.missing? // "not found"'
"not found"
```
...
```bash
# length - 获取长度
jq> echo '"hello"' | jq 'length'
5

jq> echo '[1,2,3,4,5]' | jq 'length'
5

jq> echo '{"a":1,"b":2,"c":3}' | jq 'length'
3

jq> echo 'null' | jq 'length'
0

# keys - 获取键名（对象）或索引（数组）
jq> echo '{"z":1,"a":2,"m":3}' | jq 'keys'
["a", "m", "z"]

jq> echo '[10,20,30]' | jq 'keys'
[0, 1, 2]

# 结合使用
jq> echo '{"name":"Alice","age":30,"city":"NYC"}' | jq 'keys | length'
3

jq> echo '[1,2,3,4,5]' | jq 'keys | .[]'
0
1
2
3
